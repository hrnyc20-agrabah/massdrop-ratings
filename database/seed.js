...
db.serialize(() => {
  ...
  const insertUsers = db.prepare('INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?)');
  for (let u = 1; u < 501; u++) {
    insertUsers.run(
      u, // user id
  ...
  const insertReviews = db.prepare('INSERT INTO reviews VALUES(?, ?, ?, ?, ?, ?, ?)');
  ...
  const insertComments = db.prepare('INSERT INTO comments (comment_date, comment_author_id, comment_replied_to_id, comment_review_id, comment_body, comment_likes) VALUES (?, ?, ?, ?, ?, ?)');
